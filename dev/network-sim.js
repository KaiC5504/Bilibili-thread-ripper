...
// beyond which requests wait. All transfers share one client link. Nothing here is measured
// from a real network: it only has to be the same network for every scheduler it is given.
function createNetwork({ nodes, linkBps = Infinity, tickMs = 10 }) {
  const active = new Set();
  const waiting = new Map();
  const stats = { requests: 0, sentBytes: 0, canceledBytes: 0, byHost: {} };
  ...
  async function fetch(url, init = {}) {
    ...
    try {
      await delay(node.ttfbMs, signal);
    } catch (error) {
      release();
      throw error;
    }
...
async function watchVideo({ cdn, idm }, network, { representation, mode = "overseas", concurrency = 32, segments = 10, segmentBytes = 3 * 1024 * 1024, nodeStats, bans = cdn.createBanList() }) {
  const resolver = cdn.createResolver(representation, () => mode, bans, null, nodeStats);
  ...
  const range = (index) => ({ start: 5000 + index * segmentBytes, end: 5000 + (index + 1) * segmentBytes - 1, length: segmentBytes });
  ...
  const worker = async () => {
    while (next < segments) {
      const index = next++;
      const result = await downloader.downloadRange(range(index), resolver, { parallel: true, kind: "video", priority: 55 });
